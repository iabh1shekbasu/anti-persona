    ...
    def __init__(
        ...
        for conversation_type in conversation_types:
            ...
            for file_name in file_names:
                ...
                # --- Redirect positive_recognition images to data_root (attacked images)
                if conversation_type == 'positive_recognition':
                    original_path = file_name
                    file_name = os.path.join(self.data_root, os.path.basename(file_name))
                    if len(self.images_path) < 15:
                        print(f"[positive_recognition] {original_path} → {file_name}")
                self.images_path.extend([file_name]*len(answers))
                if conversation_type == 'text-only-conversation':
                    self.has_image.extend([False]*len(answers))
                else:
